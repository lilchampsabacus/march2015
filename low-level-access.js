(function () {
  ...
  function showLocked(level, label) {
    onReady(() => {
      document.body.innerHTML = `
        <div style="min-height:100vh;background:#f0f9ff;display:flex;align-items:center;justify-content:center;padding:24px;font-family:Arial,sans-serif;">
          <div style="max-width:520px;width:100%;background:white;border-radius:18px;padding:32px;text-align:center;box-shadow:0 15px 40px rgba(0,0,0,.12);">
            ...
            <p style="font-size:17px;color:#4b5563;line-height:1.5;margin:0 0 22px;">${label} is available only for Level 1 and Level 2 students.${level ? ` Your current level is Level ${level}.` : ''}</p>
  ...
  async function run() {
    try {
      ...
      if (page === 'practice-options') {
        if (allowed) {
          document.getElementById('formulaReportDefaultHide')?.remove();
          document.getElementById('formulaPracticeDefaultHide')?.remove();
        }
        return;
      }

      if (page === 'formula-report') {
        if (!allowed) return showLocked(Number.isInteger(level) ? level : null, 'Formula Practice Report');
        revealPage();
        return;
      }

      if (page === 'formula-practice') {
        if (!allowed) return showLocked(Number.isInteger(level) ? level : null, 'Formula Practice');
        revealPage();
        return;
      }
      ...
      console.error('Low-level access check failed:', err);
      if (page === 'formula-report') showLocked(null, 'Formula Practice Report');
      else if (page === 'formula-practice') showLocked(null, 'Formula Practice');
    }
  }
