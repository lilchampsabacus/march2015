-- Level 1 can access only Abacus Grade 12.
...
begin
 ...
 where p.id=auth.uid() and p.role='student' and p.is_active is true and p.student_status='active' and a.enabled
   and (p.current_level<>1 or (a.mode='abacus' and a.grade=12));
end $$;
revoke all on function public.ige_available() from public,anon;
grant execute on function public.ige_available() to authenticated;
...
begin
 ...
 if p.current_level=1 and (m<>'abacus' or g<>12) then raise exception 'Level 1 offers Abacus Grade 12 only'; end if;
   ...
   if section='+/-' then
    ...
    if m='abacus' and g=3 and section='÷' and n>=7 then
      ...
      for tries in 1..10000 loop
        ...
        if display_divisor<>0 and display_divisor<>1 and position('0' in regexp_replace(trim_scale(abs(display_divisor))::text,'^0\.','.'))=0
    ...
    for tries in 1..1000 loop
     left_n:=public.ige_sample_pattern(spec->0);
     right_n:=public.ige_sample_pattern(spec->1);
     -- The displayed right operand is the multiplier or divisor. Never use 1.
     if right_n=1 then continue; end if;
